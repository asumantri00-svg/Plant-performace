    ...
    <div className="flex flex-col items-center justify-center p-4">
      <div className="w-64 h-64 relative">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie
              ...
              stroke="none"
            >
              <Cell fill="#001f3f" /> {/* Dark Blue for progress */}
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-32 h-32 bg-gradient-to-b from-yellow-300 to-yellow-500 rounded-full shadow-inner flex flex-col items-center justify-center border-4 border-white/30">
            <span className="text-2xl font-black text-[#001f3f] leading-none">{percentage.toFixed(2)}%</span>
      ...
      <div className="mt-4 grid grid-cols-2 gap-4 w-full">
        <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-center">
          ...
          <p className="text-sm font-black text-slate-800">{actual.toLocaleString()} Ton</p>
        </div>
        <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-center">
          <p className="text-[10px] font-black text-slate-400 uppercase mb-1">Target Budget</p>
          <p className="text-sm font-black text-slate-800">{target.toLocaleString()} Ton</p>
        </div>
      </div>
    </div>
  );
